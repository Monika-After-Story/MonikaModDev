...
label bye_prompt_shower:
    if mas_isMoniNormal(higher=True):
        m 1eua "Leaving to go take a shower?"
        if mas_isMoniLove() and renpy.random.randint(1,100) == 1:
            call bye_brb_shower_tease
        else:
            m "Alright."
            m 1hua "Have a nice shower, [player]~"
            m 3eub "Don't forget to come back!"

    elif mas_isMoniUpset():
        m 2esc "You're going to go take a shower?"
        m 2eud "Alright then."
        m 2rfc "{cps=*2}Maybe then you'll treat me a little cleaner.{/cps}{nw}"
        $ _history_list.pop()
        m 2esc "Enjoy...{w=1}I guess..."
    elif mas_isMoniDis():
        m 1eka "Oh, {w=0.5}alright."
        m "You do need to shower, after all."
        m 1rksdlc "I don't mean it like you need it {i}badly{/i} or anything, you know, but..."
        m 1eka "Just don't forget to come back, okay?"
    else:
        m 6ckc "..."

    $ persistent._mas_greeting_type = store.mas_greetings.TYPE_SHOWER
    return 'quit'