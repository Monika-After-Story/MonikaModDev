...
label monika_itabag:
    ...
    m 7eud "Ita-bags originally come from another Japanese concept called 'ita-sha'."
    m 1eud "Ita-sha is when people decorate cars with things like decals and paint schemes of similar nature to ita-bags."
    m 1esa "The word was originally an abbreviation for cars that were {i}Ita{/i}lian cars."
    m 3rsb "When people started decorating their cars with all sorts of anime related things however,{w=0.2} a pun was made from it."
    m 2eub "Today,{w=0.2} ita-sha most likely refers to the wordplay on the Japanese characters ita,{w=0.2} which means painful,{w=0.2} and sha,{w=0.2} which means vehicle."
    m 2eua "Over time, the ita-sha style has spread to things other than cars like ita-bags,{w=0.2} which is the bag equivalent of ita-sha."
    m 6rksdla "You might be wondering if ita-sha or ita-bags are harmful since it has the word 'painful' in it, {w=0.3}{nw}"
    ...
    m 4eub "So, ita-sha is a word with two possible meanings!{w=0.3} It can be referring to Italian cars... {w=0.3}{nw}"
    extend 4rksdlb "Or just cars covered in stickers that are painfully embarrassing to look at."
