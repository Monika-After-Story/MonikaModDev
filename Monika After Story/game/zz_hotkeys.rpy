...
init 10 python:
    ...
    config.overlay_screens.append("mas_detection")
        ...
        def event(self, ev, x, y, st):
            ...
            if mas_afk_time < time.time() and not mas_afk_already:
                ...
                if not mas_isRstBlk(persistent.current_monikatopic):
                    ...
                    renpy.jump("mas_inactivity_detection")
                else:
                #if can't be restarted simply call afk dialogue.
                    renpy.call("mas_inactivity_detection")
            renpy.redraw(self, 0)

        def render(self, width, height, st, at):
            return renpy.Render(0, 0)

screen mas_detection():
    add MASInactivity()
...
init 5 python:
    addEvent(
        Event(
            ...
            eventlabel="mas_inactivity_detection",
            category=["dev"],
