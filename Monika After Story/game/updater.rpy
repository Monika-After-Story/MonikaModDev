...
label update_now:
    ...
    default check_wait = 0
    if time.time()-last_updated > check_wait and updater.can_update():
        $timeout = False
        $latest_version = updater.UpdateVersion('http://s3.us-east-2.amazonaws.com/monikaafterstory/updates.json',check_interval=0,simulate="available")
        call screen update_check(Return(True),Return(False))

        if _return:
            $updater.update('http://s3.us-east-2.amazonaws.com/monikaafterstory/updates.json',simulate="available")
    return
