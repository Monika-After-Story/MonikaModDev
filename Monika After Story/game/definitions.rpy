...
#init -1 python:
    ...
    class MASUptimeSyncValidator(object):
        ...
        def __run_desync_cb(self):
            """
            Tried to run the callback, logs any errors
            """
            if self.__on_desync:
                try:
                    self.__on_desync()
        ...
        def _start(self):
            ...
            while not self.__quit:
                wait_time = float(random.randint(5, 15))
                self.__last_os_ts = time.time()

                time.sleep(wait_time)

                change = time.time() - self.__last_os_ts
                diff = abs(change-wait_time)
                print(change, diff)
                if diff > 1.0:
                    self.__desyncs.append(diff)
                    self.__log_warning("Uptimes desync, possible corruption: {}".format(diff))
