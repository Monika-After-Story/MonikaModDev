...
class ValueField(Field):
	def __init__(A,name,default=None):A.name=name;A.default=default
...
class Card16(ValueField):structcode=_D;structvalues=1
class Card32(ValueField):structcode=_C;structvalues=1
class Resource(Card32):
	cast_function='__resource__';class_name='resource'
	def __init__(A,name,codes=(),default=None):Card32.__init__(A,name,default);A.codes=codes
...
class Window(Resource):cast_function='__window__';class_name=_L
...
class Colormap(Resource):cast_function='__colormap__';class_name=_K
...
class Set(ValueField):
	structvalues=1
	def __init__(A,name,size,values,default=None):ValueField.__init__(A,name,default);A.structcode=unsigned_codes[size];A.values=values
...
class Binary(ValueField):
	...
	def pack_value(C,val):
		A=val;B=len(A)
		if C.pad:return A+_A*((4-B%4)%4),B,None
...
class String8(ValueField):
	...
	def pack_value(D,val):
		...
		C=len(A)
		if D.pad:return A+_A*((4-C%4)%4),C,None
		else:return A,C,None
	def parse_binary_value(D,data,display,length,format):
		B=data;A=length
		if A is None:return decode_string(B),''
		if D.pad:C=A+(4-A%4)%4
		else:C=A
...
class String16(ValueField):
	...
	def __init__(A,name,pad=1):ValueField.__init__(A,name);A.pad=pad
	def pack_value(D,val):
		'Convert 8-byte string into 16-byte list';A=val
		if isinstance(A,bytes):A=list(iterbytes(A))
		B=len(A)
		if D.pad:C='\x00\x00'*(B%2)
		else:C=''
		return struct.pack('>'+_D*B,*A)+C,B,None
	def parse_binary_value(D,data,display,length,format):
		B=data;A=length
		if A==_N:A=len(B)//2-1
		elif A==_M:A=len(B)//2
		...
		return struct.unpack('>'+_D*A,B[:A*2]),B[C*2:]
class List(ValueField):
	...
	def parse_binary_value(B,data,display,length,format):
		...
		if I is None:
			E=[]
			if B.type.structcode is None:
				while A:K,A=B.type.parse_binary(A,H);E.append(K)
			else:
				G=_F+B.type.structcode;F=struct.calcsize(G);C=0
				while C+F<=len(A):
					D=struct.unpack(G,A[C:C+F])
					if B.type.structvalues==1:D=D[0]
					if B.type.parse_value is None:E.append(D)
			...
			if B.type.structcode is None:
				...
				for J in range(0,I):
					D=struct.unpack(G,A[C:C+F])
					if B.type.structvalues==1:D=D[0]
					if B.type.parse_value is None:E[J]=D
	...
	def pack_value(B,val):
		...
		if B.type.structcode and len(B.type.structcode)==1:
			if B.type.check_value is not None:C=[B.type.check_value(D)for D in C]
			E=array(struct_to_array_codes[B.type.structcode],C);A=encode_array(E)
		else:
			A=[]
			for D in C:A.append(B.type.pack_value(D))
			A=''.join(A)
		if B.pad:F=len(A);A=A+_A*((4-F%4)%4)
		return A,len(C),None
...
class Object(ValueField):
	def __init__(A,name,type,default=None):ValueField.__init__(A,name,default);A.type=type;A.structcode=A.type.structcode;A.structvalues=A.type.structvalues
	...
	def check_value(G,val):
		...
		if isinstance(B,tuple):
			...
			for A in G.type.fields:
				if A.name:
					if A.check_value is None:D=B[E]
		...
		for A in G.type.fields:
			if A.name:
				if A.check_value is None:D=F[A.name]
...
class PropertyData(ValueField):
	...
	def pack_value(I,value):
		...
		H=len(C);C=C+_A*((4-H%4)%4);return C,F,D
...
class ValueList(Field):
	structcode=None;keyword_args=1;default='usekeywords'
	...
	def pack_value(C,arg,keys):
		...
		for (A,H) in C.fields:
			if A.name in B:
				D=D|H;F=B[A.name]
				if A.check_value is not None:F=A.check_value(F)
				G=struct.pack(_F+A.structcode,F);E=E+G+_A*(4-len(G))
		return struct.pack(C.maskcode,D)+E,None,None
	def parse_binary_value(D,data,display,length,format):
		E=display;A=data;F={};G=int(struct.unpack(D.maskcode,A[:D.maskcodelen])[0]);A=A[D.maskcodelen:]
		for (B,H) in D.fields:
			if G&H:
				if B.structcode:
					C=struct.unpack(_F+B.structcode,A[:struct.calcsize(_F+B.structcode)])
					if B.structvalues==1:C=C[0]
					if B.parse_value is not None:C=B.parse_value(C,E)
				else:C,I=B.parse_binary_value(A[:4],E,None,None)
				F[B.name]=C;A=A[4:]
...
class ScalarObj(object):
	def __init__(A,code):A.structcode=code;A.structvalues=1;A.parse_value=None;A.check_value=None
Card8Obj=ScalarObj(_B)
Card16Obj=ScalarObj(_D)
Card32Obj=ScalarObj(_C)
class ResourceObj(object):
	structcode=_C;structvalues=1
	def __init__(A,class_name):A.class_name=class_name;A.check_value=None
	def parse_value(D,value,display):
		B=display;A=value;C=B.get_resource_class(D.class_name)
		if C:return C(B,A)
		else:return A
WindowObj=ResourceObj(_L)
ColormapObj=ResourceObj(_K)
...
class Struct(object):
	...
	def __init__(A,*C):
		A.fields=C;A.static_codes=_F;A.static_values=0;A.static_fields=[];A.static_size=None;A.var_fields=[]
		for B in A.fields:
			if B.structcode is not None:
		...
		if A.var_fields:A.structcode=None;A.structvalues=0
	...
	def to_binary(D,*M,**E):
		...
		for A in D.fields:
			if A.name and A.name not in B:
				if A.default is None:raise TypeError('Missing required argument {0}'.format(A.name))
				B[A.name]=A.default
		...
		for A in D.static_fields:
			if isinstance(A,LengthField):
				...
				if A.check_value is not None:C.append(A.check_value(B[A.name]))
				else:C.append(B[A.name])
			elif A.check_value is not None:C.extend(A.check_value(B[A.name]))
			else:C.extend(B[A.name])
		P=struct.pack(D.static_codes,*C);Q=[H[A.name]for A in D.var_fields];return P+''.join(Q)
	def pack_value(B,value):
	...
	def parse_value(F,val,display,rawdict=0):
		...
		for A in F.static_fields:
			if not A.name:0
				...
				if A.parse_value is not None:C=A.parse_value(C,display,rawdict=E)
				D[A.name]=C
			B=B+A.structvalues
	...
	def parse_binary(E,data,display,rawdict=0):
		...
		for A in E.static_fields:
			if not A.name:0
				...
				B=G[C]
				if A.parse_value is not None:B=A.parse_value(B,H)
				for L in K:I[L]=B
...
class TextElements8(ValueField):
	string_textitem=Struct(LengthOf(_E,1),Int8(_H),String8(_E,pad=0))
	def pack_value(F,value):
		B='';E={}
		for A in value:
			...
			if isinstance(A,(tuple,dict,DictWrapper)):
				...
				while D or C:E[_H]=D;E[_E]=C[:254];B=B+F.string_textitem.to_binary(*(),**E);D=0;C=C[254:]
			else:
				if isinstance(A,Fontable):A=A.__fontable__()
				B=B+struct.pack('>BL',255,A)
		G=len(B);return B+_A*((4-G%4)%4),None,None
	def parse_binary_value(C,data,display,length,format):
		...
		return B,''
class TextElements16(TextElements8):string_textitem=Struct(LengthOf(_E,1),Int8(_H),String16(_E,pad=0))
...
class DictWrapper(GetAttrData):
	...
	def __repr__(A):return _J%(A.__class__,repr(A._data))
	def __lt__(B,other):
		A=other
...
class Request(object):
	def __init__(A,display,onerror=None,*C,**D):B=onerror;A._errorhandler=B;A._binary=A._request.to_binary(*C,**D);A._serial=None;display.send_request(A,B is not None)
	def _set_error(A,error):
		if A._errorhandler is not None:return call_error_handler(A._errorhandler,error,A)
		else:return 0
class ReplyRequest(GetAttrData):
	def __init__(A,display,defer=0,*B,**C):
		A._display=display;A._binary=A._request.to_binary(*B,**C);A._serial=None;A._data=None;A._error=None;A._response_lock=lock.allocate_lock();A._display.send_request(A,1)
		if not defer:A.reply()
	def reply(A):
		A._response_lock.acquire()
		while A._data is None and A._error is None:A._display.send_recv_lock.acquire();A._response_lock.release();A._display.send_and_recv(request=A._serial);A._response_lock.acquire()
		A._response_lock.release();A._display=None
...
class Event(GetAttrData):
	def __init__(A,binarydata=None,display=None,**B):
		C=binarydata
		if C:A._binary=C;A._data,D=A._fields.parse_binary(C,display,rawdict=1);A._data[_I]=not(not A._data[_G]&128);A._data[_G]=A._data[_G]&127
		else:
			if A._code:B[_G]=A._code
			B['sequence_number']=0;A._binary=A._fields.to_binary(**B);B[_I]=0;A._data=B
	def __repr__(A):
		D=[]
		for (B,C) in A._data.items():
			if B==_I:continue
			if B==_G and A._data[_I]:C=C|128
			D.append('%s = %s'%(B,repr(C)))
		E=', '.join(D);return _J%(A.__class__,E)
	def __lt__(B,other):
		A=other
